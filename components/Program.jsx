...
import SenjaReviews from "./ReviewsApi/SenjaReviews";
...
const Program = ({trip,type, openModal, modalIsOpen, closeModal}) => {


  //const {programBar} = useG();
  const isProgramTravel = trip.lengthType === "MULTI_DAY";
  console.log("i am main ",isProgramTravel);
  ...
  const renderMainContent = () => {
    if (isProgramTravel) {
      return (
        <React.Fragment>
          <ProgramSummary trip={trip} isProgramTravel={isProgramTravel} />
          <TripProgram trip={trip} />
          <ProgramStay trip={trip}/>
     
          <ProgramRatings />
          <ProgramCars></ProgramCars>
          <ProgramTerms trip={trip} />
          <ProgramCancel openModal={openModal} />
          <ProgramFaq commonQuestions={trip.commonQuestions}/>
        </React.Fragment>
      );
    } else {
      return (
       ...
       <DailyTripProgram
  dailyTripProgram={{
    programTrip: trip.programSections?.map((section) => ({
      titleStation: section.destination?.title || "بدون عنوان",
      detailsStation: section.subDestinations?.edges?.map(
        (edge) => edge.node.title
      ),
    })) || [],
    activities: trip.activities?.map((a) => a.title) || [],
  }}
/>
...
trip.importantInfos?.map((info)=>info.title || [])

            }
            doesNotInclude={
              trip.exclusions?.map((exclusion) => exclusion.title) || []
            }
          />
          <ProgramCars />
          <ProgramRatings />
  ...
  const renderSideContent = () => (
    <ProgramSider trip={trip} shouldRenderMobileView={shouldRenderMobileView} />
  );
  ...
  const renderInfoBar = () =>
    shouldRenderMobileView ? (
      <React.Fragment>
        ...
        />
        {isProgramTravel ? <ProgramInfoBar /> : <ProgramInfoBarDaily />}
      </React.Fragment>
    ) : isProgramTravel  ? (
      <ProgramInfoBar />
    ) : (
