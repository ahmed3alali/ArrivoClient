...
const TravelsPrograms = ({trips, data}) => {
  ...
  const transformTripsData = (tripsEdges) => {
    return tripsEdges.map(({ node }) => {
      const groupSize = node.groupSize || { from: 1, to: 7 };
      const subType = node.subTypes?.[0]?.type || "withDriver";
      const province = node.provinces?.[0]?.name || "غير محدد";
  
      return {
        id: node.id,
        img: node.cardThumbnail || node.thumbnails?.[0]?.image || "/default.jpg",
        title: node.title,
        ...
        mainType: "programs",
        subType: subType,
        longTimeTravel: node.dayPrograms?.length || 1,
        rating: { avg: 4.5, count: 10 },
        tripStations: node.dayPrograms?.map(p => p.title).filter(Boolean) || [],
        offer: node.offerType || "",
        country: province,
      };
    });
  };
  ...
  const transformedTrips = multiTrips.length > 0 ? transformTripsData(multiTrips) : data; 
